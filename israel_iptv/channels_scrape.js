...
(async ()=>{
  ...
  const it_url = 'https://famelack.com/il';
  const channels_list = []
...
//file structure
  const fileStructure = ["#EXTM3U",...m3uVideoList.map((link, i)=>`#EXTINF:-1 tvg-id="ext" group-title="HEBREW",${m3uNameList[i]} \n ${link}`)].join("\n")

//write m3u8 file
  fs.writeFileSync('./israel_channels.m3u8', fileStructure)
